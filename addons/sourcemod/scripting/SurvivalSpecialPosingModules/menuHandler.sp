...
void DrawMainMenu(int client)
{
	...
	//menu.AddItem("Random pose & slay", "Random pose & slay");
...
public int MainMenu_Callback(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(StrEqual(sItem, "Random pose & slay"))
			{
				// treat as end-round for testing random poses
				slaysurvivors();
				bLoadCfgFile(-1, true);
				DrawMainMenu(param1);
			}
...
void LoadPoseMenu(int client)
{
	...
	{
		g_hArray_ConfigIDs.GetString(i, sName, sizeof(sName));
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			else
			{
				...
				{
					PrintToChat(param1, "[SM] Error loading config \x04%i\x01.", iConfig);
				}
			}
